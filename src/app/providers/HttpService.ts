...
export class HttpService extends HttpHelper {
    ...
    public doRequest(options, setting: RequestSetting) {
        const defaultSetting = HttpHelper.getDefaultSetting(setting);console.log("defaultSetting",defaultSetting.useDefaultApi);
     ...
     */
    public defaultRequest(options, setting: RequestSetting): Observable<any> {console.log("defaultRequest");
        ...
        return Observable.create(observer => {
            this.request(options, setting).subscribe(res => {
                observer.next(res.result); // data是主api约定返回的数据
            }, err => {
                observer.error(err);
            });
        });
    ...
    public request(ops, set: RequestSetting): Observable<any> {console.log("request");
        const options = {
            url: '',
            method: 'POST',
            body: null,
        ...
        return Observable.create(observer => {
            ...
            this.http.request(options.method, options.url, options).pipe(
                ...
                observer.next(res);console.log("res:",res);
